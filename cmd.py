...
def list_plugins(args, config, pattern='*'):
    ...
    for plugin in sorted(available_not_installed):
        ...
        available_version = _extract_version(available[plugin])
        try:
            version_str = '.'.join(available_version)
        except TypeError:
            version_str = "unknown"  # Or some other appropriate default value
        
        print(line.format(name=plugin, width=max_len, version=version_str, enabled='-', status='available'))
